...
import { Globe, RefreshCw, Compass, MapPin, Target, Activity, Zap, ZoomIn, ZoomOut, Shield, Cpu } from 'lucide-react';
import * as THREE from 'three';
import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls.js';

// Modern THREE.Timer custom implementation to replace the deprecated THREE.Clock
class ThreeTimer {
...
// Ensure THREE.Timer is safely provisioned on the namespace
if (!(THREE as any).Timer) {
  (THREE as any).Timer = ThreeTimer;
}
...
export default function ImmersiveGlobeCanvas({
  ...
  const texturesRef = useRef<{
    ...
    specularTexture: THREE.CanvasTexture | null;
    bumpTexture: THREE.CanvasTexture | null;
    nightLightsTexture: THREE.CanvasTexture | null;
    cloudTexture: THREE.CanvasTexture | null;
  } | null>(null);

  // Resize listener to adapt viewport size
  useEffect(() => {
  ...
  // Fetch standard global land boundaries
  useEffect(() => {
    let isMounted = true;
  ...
  // Pre-generate and cache the heavy procedural textures exactly once when worldData is loaded
  useEffect(() => {
    if (!worldData) return;
    ...
    return () => {
      if (texturesRef.current) {
        if (texturesRef.current.dayTexture) texturesRef.current.dayTexture.dispose();
        if (texturesRef.current.specularTexture) texturesRef.current.specularTexture.dispose();
        if (texturesRef.current.bumpTexture) texturesRef.current.bumpTexture.dispose();
        if (texturesRef.current.nightLightsTexture) texturesRef.current.nightLightsTexture.dispose();
        if (texturesRef.current.cloudTexture) texturesRef.current.cloudTexture.dispose();
  ...
  useEffect(() => {
    if (!worldData || !mountRef.current || !texturesRef.current) return;
    ...
    // Procedural multi-layer textures loaded from cache
    const { dayTexture, specularTexture, bumpTexture, nightLightsTexture, cloudTexture } = texturesRef.current;
    ...
    const clock = new (THREE as any).Timer();
    ...
    const animate = () => {
      ...
      // Real-time Solar Tracking Orbital Engine
      try {
        const now = new Date();
        const utcHours = now.getUTCHours();
        const utcMinutes = now.getUTCMinutes();
        const utcSeconds = now.getUTCSeconds();
        const dayFraction = ((utcHours * 3600) + (utcMinutes * 60) + utcSeconds) / 86400; // 0 to 1
        
        // Solar azimuth angle based on day fraction
        const solarAngle = dayFraction * Math.PI * 2;
        const orbitRadius = 8;
        
        // Tilted circular orbit to represent seasonal solar declination
        const sunX = orbitRadius * Math.cos(solarAngle);
        const sunZ = orbitRadius * Math.sin(solarAngle);
        const sunY = 3.5 * Math.sin(solarAngle); // 23.5 degrees tilt equivalent
        
        sunLight.position.set(sunX, sunY, sunZ);
      } catch (solarErr) {
        console.warn("[Solar Tracking] Failed to update dynamic sun position:", solarErr);
      }
      ...
      const currentMarkers = landmarksRef.current.map((loc) => {
  ...
  // Resize handler to update camera aspect and renderer size without recreating scene
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const targetVector = convertLatLngToVector3(explorerState.lat, explorerState.lng, 1.5);
      ...
      <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4 z-10 bg-stone-900/40 border border-stone-800/40 p-3 rounded-2xl backdrop-blur-md mb-2">
        ...
        <div className="flex flex-col gap-1 font-mono text-[9px] text-cyan-400">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-stone-400">
            <div>LAT: <span className="text-white font-bold">{explorerState.lat.toFixed(4)}&deg;</span></div>
            <div>LNG: <span className="text-white font-bold">{explorerState.lng.toFixed(4)}&deg;</span></div>
          </div>
          ...
          <div className="relative overflow-visible" style={{ width: dimensions.width, height: dimensions.height }}>
            ...
            <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
              {projectedMarkers.map((marker) => {
                ...
                const isBeaconActive = explorerState.activeLandmarkId === marker.id;

                return (
                  <div
                    key={`beacon-overlay-${marker.id}`}
                    style={{
                      position: 'absolute',
                      left: marker.x,
                      top: marker.y,
                  ...
                  >
                    ...
                    <div 
                      ...
                      style={{
                        width: isBeaconActive ? '40px' : '20px',
                        height: isBeaconActive ? '40px' : '20px',
                        animationDuration: isBeaconActive ? '1.5s' : '3.0s'
                      }}
                    />
                    ...
                    <button
                      ...
                      className={`w-4 h-4 rounded-full flex items-center justify-center transition-transform hover:scale-125 shadow-lg border border-white ${
                        isBeaconActive ? 'bg-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.6)]' : 'bg-cyan-500/80'
                      }`}
                    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs font-serif italic text-white truncate max-w-[180px] block font-bold leading-tight">
              {landmarks.find(l => l.id === explorerState.activeLandmarkId)?.title || 'Global Sweep'}
            </span>
          </div>
        </div>
